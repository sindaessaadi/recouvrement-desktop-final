...
@Component({
  selector: 'app-mode-par-tranche',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './mode-par-tranche.component.html',
  styleUrl: './mode-par-tranche.component.scss',
})
export class ModeParTrancheComponent implements OnChanges {
  @Input() principal = 0;
  @Input() frais = 0;
  @Input() taxe = 0;
  @Input() devise = 'DT';
  ...
  // Les champs montant/frais/taxes sont saisis par l'utilisateur en DT (comme partout ailleurs
  // dans l'app), alors que principal/frais/taxe (les @Input) sont en millimes : conversion ici.
  ...
  updateRow(i: number, field: keyof TrRow, val: string): void {
  ...
  handleAmountChange(i: number, field: keyof TrRow, val: string): void {
  ...
  get sumOtherMontant(): number {
    return this.rows.slice(0, Math.max(0, this.n - 1)).reduce((s, r) => s + this.millimes(r.montant), 0);
  }

  get lastMontant(): number {
    return this.n > 0 ? Math.max(0, this.principal - this.sumOtherMontant) : 0;
  }

  get depassement(): boolean {
    return this.sumOtherMontant > this.principal;
  }

  getMontant(i: number): number {
    return i === this.n - 1 ? this.lastMontant : this.millimes(this.rows[i]?.montant ?? '');
  }
  ...
  private emettre(): void {
    ...
    this.validChange.emit(this.n > 0 && this.toutCorrect);
  }
}